import{g as l,S as g,r as p,i as S,a as k,b as A,c as T}from"./main-DpI-GIT5.js";l.registerPlugin(g);const d=[{id:"01",title:"MONOLITHIC CONCRETE ENGINEERING",subtitle:"ULTRA-HIGH PERFORMANCE FIBER CONCRETE (UHPFRC) & DIRECT POUR GRIDS",description:"Our core discipline. We cast uninterrupted load-bearing concrete monoliths up to 1,200 cubic meters per continuous pour. By blending amorphous silica fume with micro-steel tensile fibers, our UHPFRC eliminates the need for secondary crack waterproofing and achieves compressive ratings up to 180 MPa while preserving crisp, self-supported 90-degree neobrutalist corners.",specs:[{label:"COMPRESSIVE YIELD RATING",value:"180 MPa (ASTM C1856)"},{label:"CONTINUOUS POUR VELOCITY",value:"120 - 240 M³ / HR"},{label:"CORNER EDGE TOLERANCE",value:"±0.1 MM / UNCHIPPED"},{label:"MICRO-STEEL FIBER DENSITY",value:"3.2% BY VOLUME"}],svgContent:`
    ...
    `},{id:"02",title:"SEISMIC & DYNAMIC LOAD REINFORCING",subtitle:"ELASTOMERIC BASE ISOLATION & VISCOUS SHEAR DAMPENERS",description:"In tectonically active zones, mass alone is insufficient. We engineer subterranean elastomeric raft bearings and hydraulic diagonal shear dampeners that isolate kinetic ground waves from the superstructure. During a Class IX seismic shock, the building core glides smoothly up to ±650 mm horizontally without transmitting shear stress to upper floor slabs.",specs:[{label:"SEISMIC RATING CERTIFICATION",value:"CLASS IX-EX TECTONIC"},{label:"HORIZONTAL RAFT DISPLACEMENT",value:"±650 MM NOMINAL"},{label:"DAMPENING RECOVERY SPEED",value:"0.8 SECONDS / WAVE"},{label:"BEARING MATERIAL MATRIX",value:"LEAD-RUBBER LAMINATE"}],svgContent:`
    ...
    `},{id:"03",title:"HEAVY STEEL STRUCTURAL FABRICATION",subtitle:"UNPAINTED OXIDIZED CORTEN EXOSKELETONS & BOX TRUSSES",description:"Where structural spans exceed 30 meters, we deploy heavy Corten steel box trusses. Unpainted by design, our high-tensile steel forms a protective self-sealing oxidized patina within 12 months of exposure. This raw rust armature provides unyielding tensile rigidity (`800 MPa`) while contrasting dramatically with pristine gray board-formed concrete cores.",specs:[{label:"TENSILE YIELD CAPACITY",value:"800 MPa GRADE STEEL"},{label:"UNSUPPORTED CLEAR SPAN",value:"UP TO 64 METERS"},{label:"OXIDATION SEAL TIMELINE",value:"12 MONTHS PASSIVE"},{label:"WELDING JOINT TOLERANCE",value:"ULTRASONIC 100% TESTED"}],svgContent:`
    ...
    `},{id:"04",title:"SUBTERRANEAN FOUNDATION BLUEPRINTING",subtitle:"DEEP CAISSON PILES & BASALT BEDROCK ANCHORING",description:"Every monolith requires an unmovable root system. We drill high-density reinforced caisson piles up to 60 meters deep into solid granite or basalt bedrock. Combined with post-tensioned soil nails and continuous bentonite slurry walls, our subterranean vaults remain completely impervious to hydrostatic groundwater pressure and soil liquefaction.",specs:[{label:"CAISSON PILE DRILL DEPTH",value:"UP TO -65 METERS"},{label:"BEDROCK ANCHOR TENSILE RATING",value:"4,500 kN / ANCHOR"},{label:"HYDROSTATIC SHIELDING",value:"10 BAR WATERPROOF"},{label:"SOIL LIQUEFACTION FACTOR",value:"ZERO SHEAR LOSS"}],svgContent:`
    ...
    `},{id:"05",title:"PRECAST HIGH-DENSITY FAÇADE SYSTEMS",subtitle:"THERMAL MASS WALL PANELS & DEEP LOUVER MATRICES",description:"Instead of glass curtain walls that leak energy, we fabricate massive precast concrete façade panels weighing up to 18 tonnes each. Deep angled louvers cast directly into the panel geometry block high-angle summer solar radiation while letting low-angle winter sunlight penetrate deep into interior floors, cutting cooling demands by 64%.",specs:[{label:"SINGLE PANEL WEIGHT MASS",value:"UP TO 18.5 TONNES"},{label:"SOLAR HEAT GAIN CUT",value:"-64% PASSIVE SHIELD"},{label:"THERMAL STORAGE LAG",value:"11.5 HOURS DIURNAL"},{label:"INTERLOCK MOUNTING JOINT",value:"DOUBLE NEOPRENE SEAL"}],svgContent:`
    ...
    `},{id:"06",title:"ACOUSTIC & THERMAL MASS ISOLATION",subtitle:"ANECHOIC CONCRETE BUNKERING & VIBRATION DAMPING",description:"Designed for sovereign data installations and research vaults requiring complete electromagnetic and acoustic isolation. By casting dual-skin concrete walls separated by a 300mm evacuated acoustic air gap and heavy basalt rock wool, exterior sonic pressure (up to 120 dB) is completely neutralized inside the core.",specs:[{label:"SONIC DAMPENING RATING",value:"100 dB+ TRANSMISSION LOSS"},{label:"DUAL WALL THICKNESS",value:"800 MM TOTAL MATRIX"},{label:"AIR GAP VACUUM SEPARATION",value:"300 MM ISOLATED CAVITY"},{label:"ELECTROMAGNETIC SHIELDING",value:"MIL-STD-188-125 EMP"}],svgContent:`
    ...
    `}];document.addEventListener("DOMContentLoaded",()=>{p("capabilities"),S(),k(),A(".mask-text"),T();const s=document.getElementById("services-list"),E=document.getElementById("service-id"),y=document.getElementById("service-title"),u=document.getElementById("service-subtitle"),h=document.getElementById("service-description"),n=document.getElementById("service-svg"),I=document.getElementById("spec-table-body");function a(o){const e=d[o];s.querySelectorAll(".service-row").forEach((t,r)=>{r===o?t.classList.add("active"):t.classList.remove("active")}),l.to(["#service-title","#service-subtitle","#service-description","#spec-table-container"],{opacity:0,y:-10,duration:.2,onComplete:()=>{E.textContent=`[ SERVICE SPECIFICATION // #${e.id} ]`,y.textContent=e.title,u.textContent=e.subtitle,h.textContent=e.description,I.innerHTML=e.specs.map(t=>`
          <tr>
            <td style="font-weight: 700;">${t.label}</td>
            <td>${t.value}</td>
          </tr>
        `).join(""),l.to(["#service-title","#service-subtitle","#service-description","#spec-table-container"],{opacity:1,y:0,duration:.4,ease:"power2.out"})}}),n.innerHTML=e.svgContent;const c=n.querySelectorAll("path, line, rect, circle, polygon");c.forEach(t=>{const r=t.getTotalLength?t.getTotalLength():800;t.style.strokeDasharray=r,t.style.strokeDashoffset=r}),l.to(c,{strokeDashoffset:0,duration:1.4,stagger:.05,ease:"power3.inOut"})}s&&(s.innerHTML=d.map((e,i)=>`
      <div class="service-row ${i===0?"active":""}" data-index="${i}">
        <div class="service-index">[${e.id}]</div>
        <div class="service-title">${e.title}</div>
      </div>
    `).join(""),s.querySelectorAll(".service-row").forEach((e,i)=>{e.addEventListener("click",()=>a(i))}),a(0))});
